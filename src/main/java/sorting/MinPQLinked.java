...
public class MinPQLinked<Key>
{
     ...
     */
    public MinPQLinked(Comparator<Key> comparator) { this.comparator = comparator; }
    ...
    private Node createNodeInLastLayer()
    {
        ...
        while (current.left != null && current.right != null)
        {
            ...
            } else current = current.left; // follow left direction
    ...
    {
        Key minKey = min();
        if (size() == 1) { root = null; return minKey; }
    ...
    {
        Node current = root;
        if (size() == 1) { root = null; return current.value; }

        current.size--;
        while (current.left != null)
        {
            if (!isPowerOfTwo(current.left.size + 1) || current.right == null) current = current.left;
            else
            {
                if (!isPowerOfTwo(current.right.size + 1)) current = current.right;
                else
                {
                    if (current.left.size > current.right.size) current = current.left;
    ...
    private void sink(Node n)
    {
        while (n.left != null)
        {
            ...
            if (n.right != null && greater(n.left, n.right)) minChild = n.right;
    ...
    private boolean isPowerOfTwo(int x) { return (x & (x - 1)) == 0; }

    // Check if node i > j
    private boolean greater(Node i, Node j) { return comparator.compare(i.value,j.value) > 0; }
