...
public class TaskScheduler
{
     ...
     */
    public void addTask(String task, List<String> dependencies) { this.graph.put(task, dependencies); }
    ...
    public boolean isValid(List<String> schedule)
    {
        ...
        {
            if (already_scheduled.contains(dependence)) return false;

            List<String> listDependencies = this.graph.getOrDefault(dependence, null);

            if (listDependencies == null) return false;

            for (String task : listDependencies)
            {
                if (!already_scheduled.contains(task)) return false;
            }
