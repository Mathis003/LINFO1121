...
public class PreorderToBST
{
    ...
    public Node preorderRead(int[] preOrderInput, int i, int min, int max) {
        ...
        // To switch between root's left and root's right branch
        if ((preOrderInput[i] < min) || (max < preOrderInput[i])) return null;
        ...
        Node right = preorderRead(preOrderInput, i + leftSize + 1, preOrderInput[i], max);

        return new Node(left, right, preOrderInput[i]);
    }
    // BEGIN : STUDENT

    public int[] preorderWrite() { return root == null ? new int[0] : root.preOrder(); }

    protected static int size(Node x) { return x == null ? 0 : x.size; }

    @Override
    public String toString() { return root == null ? "empty" : root.toString(); }
    ...
    public static class Node
    {
        ...
        @Override
        public String toString() { return "new Node("+left+","+right+","+key+")"; }
